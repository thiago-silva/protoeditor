...
#ifndef DEBUGGERPYTHONDBGP_H
#define DEBUGGERPYTHONDBGP_H
...
class PythonDBGPSettings;
class PythonDBGPNet;
class DebuggerExecutionPoint;


class DebuggerPythonDBGP: public AbstractDebugger
{
  Q_OBJECT
public:
  enum { InfoMsg, WarningMsg, ErrorMsg };

  DebuggerPythonDBGP(LanguageSettings*);
  virtual ~DebuggerPythonDBGP();
...
//   virtual void init();
  ...
  PythonDBGPSettings* settings();
public slots:
  void slotSettingsChanged();

private slots:
  void slotStarted();
  void slotStopped(); //end of debug
  void slotStepDone();
...
private:
  ...
  PythonDBGPSettings* m_pdbgSettings;
  PythonDBGPNet* m_net;  
